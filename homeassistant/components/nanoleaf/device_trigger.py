...
async def async_get_triggers(
    ...
    triggers: list[dict] = [{**base_trigger, CONF_TYPE: NanoleafTriggers.LAYOUT_CHANGE}]

    if device_entry.model in ("NL29", "NL42", "NL52"):
        for trigger_type in TOUCH_TRIGGER_TYPES:
...
async def async_attach_trigger(
    ...
    """Attach a trigger."""
    print("Device ID: ", config[ATTR_DEVICE_ID])
    event_config = event_trigger.TRIGGER_SCHEMA(
        {
            ...
            event_trigger.CONF_EVENT_DATA: {
                ...
                CONF_DEVICE_ID: config[ATTR_DEVICE_ID],
            },
        }
    )
